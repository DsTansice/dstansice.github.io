...
;(function($) {
    ...
    avgRuntime.prototype.show = function(block, dialog) {
        ...
        var i = 0;
        //检索输出中是否存在图片，存在的话跳过特效
        if(!that.content.match("img")==""){ s.innerHTML=that.content; i=100; };
        ...
        timer=setInterval(function(){
            ...
            i++;
            //完成后执行
            if(s.innerHTML==that.content||i>100){
                clearInterval(timer);
    ...
    avgRuntime.prototype.action = function(obj) {
        ...
        switch (obj.action) {
            // 动作 继续-等待用户点击
            case "next":
                var option = $(".avgplayer .selector").append('<a class="opition" href="javascript:;"><i class="icon-right-dir"></i></a>').fadeIn("290");
    ...
    avgRuntime.prototype.effect = function(obj) {
        ...
        switch (obj["effect"]) {
            ...
            case 'dark':
                ...
                $("*:not(.avgplayer)").css("background", "#000");
                $("*:not(.avgplayer) .avgplayer *").css({"color": "#fff", "zoom": "1.2"});
                $("*:not(.avgplayer) .avgplayer a:hover").css("color", "#ff8484");
                break;

            case 'flash':
                this.log('运行flash特效', 'Effect');

                $(".effectlayer").addClass("effect_flash");
                break;
        }
    }
