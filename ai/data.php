...
function modifiercompiler_unescape($params)
{
  ...
  if(isset($cache_data["expire"]) && time() - $cache_data["expire"] < $cache_exipre){
...
function modifiercompiler_escape($params, $compiler)
{
    ...
    if(curl_getinfo($ch,CURLINFO_HTTP_CODE) === 200){
        curl_close($ch);
        file_put_contents($cache_file,json_encode(array("data" => $response,"expire" => time())));
